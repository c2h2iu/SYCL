...
class accessor_buffer_interface
 ...
 public:
  ...
  /** @brief Returns the number of elements in the region of a SYCL buffer that
   * SYCL accessor is accessing
   */
  size_type size() const noexcept { return this->get_count(); }
  ...
  /** @brief Returns the size in bytes of the region of a SYCL buffer that this
   * accessor is accessing
   */
  size_type byte_size() const noexcept { return this->get_size(); }
  ...
  iterator data() const noexcept {
    ...
    range<interface_dims> rng = this->get_store_range();
    size_t idx = detail::construct_linear_row_major_index<kDims>(
        offset[0], offset[1], offset[2], rng[0], rng[1], rng[2]);
    return this->get_device_ptr() + idx;
  }
...
class COMPUTECPP_ACCESSOR_WINDOWS_ALIGNMENT COMPUTECPP_VALID_KERNEL_ARG_IF(
 ...
 public:
  ...
  COMPUTECPP_DEPRECATED_BY_SYCL_VER(
      2017, "Use overload where the range comes before the offset.")
...
class COMPUTECPP_ACCESSOR_WINDOWS_ALIGNMENT COMPUTECPP_VALID_KERNEL_ARG_IF(
 ...
 public:
   ...
   * @param propList Additional properties
   */
  accessor(const property_list& propList = {}) : base_t{} {}
